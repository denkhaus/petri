...
`pre_tool_use` hooks block inside a child; a child never gets the question
tool, project memory files, or skill directories; a child may delegate again
within the open-session bound. `wait` blocks until the child finishes; a
