//! Run a lowered Fabro graph under the stub registry, with every sandbox
//! simulated (no provider plugin is involved), with byte-identical replay,
//! and read the result the way the plan's acceptance items do: the path
//! taken, each node's Fabro outcome, the final `kv`, and the run status.
...
pub fn runs_report(results: &[(String, RunResult)], pin: &str) -> String {
    ...
    let _ = writeln!(
        out,
        "Every corpus workflow that lowers, run end to end under the stub registry (Fabro's \
         `--dry-run` outcomes: every stage succeeds, a human gate takes its first choice) with \
         every sandbox simulated, so no provider plugin is involved, and byte-identical replay \
         verified. Fabro at `{pin}`. Generated by \
