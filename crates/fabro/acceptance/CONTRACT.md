...
| A machine-readable record per scenario with pins, launch, matched services, raw and normalized observations, final context, artifacts, output, assertions, decisions, cleanup, library links | `tests/support/fabro/evidence.rs` (`Recorder`), proven by `fabro_evidence_blackbox.rs` | met for the writer; partial for coverage: only the smoke scenario records today; task 17 wires `Recorder` into every required scenario and task 18 fills `compatibility.differential` |
