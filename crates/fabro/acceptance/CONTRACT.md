...
1. Fan-out scale: the fork snapshot is copied per child, so records, time
   and memory grow with N squared (above). Live children are bounded to
   `max_parallel`. Owner Petri core.
