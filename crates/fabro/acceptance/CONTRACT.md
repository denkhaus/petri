...
| `component` with `for_each`, `max_parallel`, `tripleoctagon` fan-in | `petri-fabro-steps::parallel` (`static_branches_return_envelopes_that_never_merge_into_the_parent`, `mixed_failures_join_partially_and_all_failed_fails_the_fan_in`, `an_empty_for_each_list_joins_with_no_branches_and_no_child`, `a_repeated_fork_publishes_results_per_visit_with_its_own_children`, `a_nested_fork_runs_inside_its_branch_and_reports_its_own_results`, `cancelling_the_run_settles_every_branch_child`, `resume_keeps_a_finished_branch_and_finishes_the_unfinished_one`), `petri-execution::admission` (`one_slot_serializes_the_attempts_of_a_fork_and_two_slots_let_them_overlap`, `a_backoff_releases_the_fork_slot_so_a_queued_branch_runs_first`, `the_limit_counts_finished_children_and_names_the_refused_call`), `fabro_blackbox::for_each_branches_keep_distinct_values_under_one_key_in_item_order`, `fabro_blackbox::an_empty_for_each_list_joins_without_calling_the_model`, `fabro_blackbox::nested_joins_report_the_inner_results_inside_the_outer_envelope`, `fabro_blackbox::parallel_gates_bind_each_answer_to_its_own_branch`, cells `backend/dynamic-parallelism@host` and `@docker`, both readiness suites. Accepted differences `parallel-branches-in-path`, `branch-context-updates-are-a-diff`, `empty-for-each-placeholder`. **Gap (scale), owner Petri core**: two successive 1,000-item forks (`petri-fabro-acceptance::e2e::two_successive_thousand_item_forks_stay_under_the_ceiling`, `#[ignore]`). Since 2026-09-07 a fork has at most `max_parallel` live children (`petri-execution::admission::a_fork_of_fifty_children_keeps_four_live_and_starts_them_in_order`, `cancelling_the_parent_finishes_every_queued_child_as_cancelled`, `resume_redispatches_queued_children_under_the_bound`); that removed the idle child drivers (300 items: peak RSS 1.45 GB to 1.05 GB, wall time unchanged at about 40 s). Since the same day the coordinator store applies each record in place instead of cloning its whole state per append (`petri-execution::store::tests::a_rejected_event_leaves_the_state_and_the_log_untouched`, `a_failed_write_leaves_the_state_unchanged_and_is_reported`); that removed the N squared time: one 1,000-item fork finishes in 42 s (before, 211 of 1,000 children in 280 s), 300 items in 10.5 s (from 40 s), and per-child time is flat at 33 to 46 ms from 100 to 1,000 items, about three quarters of it `fsync`. What remains is the O(N) fork snapshot copied per child: three durable records per child (`InvocationDeclared`, `ExecutionDeclared`, the `InvocationFinished` context), the resolved branch config, and the waiting branch firing on the parent side. `coordinator.jsonl` and memory still grow with N squared (29 KB per child at 100 items, 81 KB at 300, 266 KB at 1,000; 7.2 GB peak RSS for one 1,000-item fork; the two-fork test's run now completes in 132 s at 39 GB with a 1.66 GB log, and its `parallel.results` is not offloaded). The next step is to offload the `for_each` source list and `parallel.results` to the blob store so the per-child copies shrink (`.ai/reviews/fabro-unified/fanout-append-clone.md`). The 10,000-invocation ceiling itself is proven by `exactly_ten_thousand_invocations_are_admitted_and_the_next_is_refused` in `test:long` |
...
1. Fan-out scale: the fork snapshot is copied per child, so records and
   memory grow with N squared (above); time is linear in N since the store
   applies records in place. Live children are bounded to `max_parallel`.
   Owner Petri core.
