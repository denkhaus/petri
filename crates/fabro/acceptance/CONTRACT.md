...
| `[run.agent]` keys | `skills`, `subagents`, `compaction` and any key other than `fabro_tools` and `mcps` are refused (`unsupported.workflow_toml.key`): the pinned Fabro's `[run.agent]` denies unknown keys and has no setting for skills, sub-agents or compaction; sub-agents are always on for native agents, so the `subagents` refusal says so | refused by the parser |
