...
//! (`crates/fabro/oracle/expected/<case>.json`), generated by running the
//! pinned Fabro binary through `scripts/oracle-regenerate.sh`. Every fixture
//! must name the pinned Fabro commit; a fixture from another revision is
//! rejected, so fixtures and parity runs always use one reference version. A
//! case whose fixture records a deliberate departure is compared with the
//! departure recorded.
...
/// A fixture whose Fabro result is `rejected` (Fabro's validator refused the
/// workflow) must carry a departure: Petri accepting the spelling is itself a
/// recorded difference.
...
async fn every_case_matches_the_fabro_oracle() {
    ...
    for case in cases() {
        ...
        let commit = fixture["fabro_commit"].as_str().unwrap_or("");
        if commit != pinned {
            wrong_revision.push(format!("{}: fixture is from `{commit}`", case.name));
            continue;
        }
        ...
        let fabro_rejected = fixture["fabro"]["status"] == "rejected";
        assert!(
            !fabro_rejected || case.departure.is_some(),
            "{}: Fabro rejected the workflow but the case records no departure",
            case.name
        );
    ...
    assert!(
        wrong_revision.is_empty(),
        "every fixture is generated from the pinned Fabro `{pinned}`; mixed revisions are \
         rejected. Run scripts/oracle-regenerate.sh:\n  {}",
        wrong_revision.join("\n  ")
    );
    assert!(
        mismatches.is_empty(),
