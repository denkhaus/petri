# The fake ACP agent Fabro ships for its own tests, copied verbatim from
# lib/components/fabro-acp/src/test_support.rs (fake_acp_agent_script) at the
# pinned Fabro commit in crates/fabro/corpus-pin.txt. Petri's agent-step and
# end-to-end tests launch it as the ACP command. Test data, not a build input:
# `scripts/oracle-regenerate.sh --refresh-fake-agent` re-extracts it when the
# pin moves, and `crates/fabro/acceptance/tests/routing.rs` checks that this
# copy still matches the pinned source when the corpus is fetched.
import json
import os
import signal
import sys
