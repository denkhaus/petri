...
46 workflows from the Fabro repository at `05ebd0fd1beec214b558f4b478e36bd08b507dc7`: 32 Fabro workflows and demos, and 14 Attractor-dialect fixtures under `test/attractor/`, kept so the dialect boundary stays tested: a fixture with an Attractor spelling is an expected `unsupported.legacy_dialect` rejection, and one within the Fabro subset lowers. Generated by `cargo nextest run -p petri-fabro-acceptance --test harness` after `scripts/corpus-fetch-fabro.sh`.
...
| Code | Occurrences |
|---|---|
| `attractor.internal_context` | 1 |
| `attractor.prompt_missing` | 40 |
...
| `.fabro/workflows/code-review/code-review.fabro` | warnings | 26 | `attractor.internal_context`, `ignored.workflow_toml.run.artifacts`, `ignored.workflow_toml.run.checkpoint`, `ignored.workflow_toml.run.integrations`, `ignored.workflow_toml.run.pull_request`, `ignored.workflow_toml.run.run_branch` |
