...
use crate::{ContainerName, ContainerPrefix, prepare_image, run_docker, sweep_prefixed};
...
pub(crate) fn service_prefix(base: &ContainerName) -> ContainerPrefix {
    ContainerPrefix::new(format!("{base}-svc-"))
}
...
pub(crate) async fn sweep(base: &ContainerName) {
    sweep_prefixed(&service_prefix(base)).await;
    let _ = run_docker(&["network", "rm", base.as_str()]).await;
}
...
pub(crate) async fn realize(
    ...
    match realize_inner(scope, base, publish_ports, ctx).await {
...
async fn realize_inner(
    ...
    run_docker(&["network", "create", base.as_str()]).await?;
    ...
    for service in &scope.services {
        ...
        let ctx = ctx.clone();
        pulls.spawn(
            async move { prepare_image(&image, credentials.as_ref(), scope_id, &ctx).await },
        );
    }
    let mut failed = None;
    ...
    for service in &scope.services {
        let name = service_prefix(base).join(&service.name);
...
/// itself.
async fn await_health(container: &ContainerName, service: &SmolStr) -> Result<(), EnvError> {
...
fn report_unhealthy(service: &SmolStr, container: &ContainerName, reason: &'static str) {
    tracing::warn!(
        service = %service,
        container = %container,
        reason,
...
async fn service_failure(service: &SmolStr, container: &ContainerName, what: &str) -> EnvError {
    let logs = run_docker(&["logs", "--tail", "10", container.as_str()])
        .await
        .unwrap_or_default();
