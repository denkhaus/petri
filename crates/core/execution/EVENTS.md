...
| `scope.released` | `ScopeReleased` | `invocation` (the lease's owner), `lease`, `scope` (the coordinator's stable `ScopeIdentity`: `{"declared": <scope>}` or `{"spliced": [..]}`), `workspace`, `provider`, `instance` (the provider's id for the sandbox; absent when none was created), `outcome` (`succeeded` or `failed`, the invocation's status as retention reads it), `retained` (whether the sandbox and its workspace still exist on the provider afterwards: stopped and kept, or deleted), `problems` (what the release could not do; the sandbox is then still there and the next release, `finish` or `petri sandbox prune`, tries again). Appended once per lease the finished invocation owned, after the executor released it, before `run.finished`; a lease a crash left live is released by the resumed run's end and recorded after `run.finished`. An inherited invocation owns no lease and records none. `context.invocation` is the owner. Nothing derived |
...
| `scope.acquired` | external | `ScopeAcquired` | `scope`, `lease` (when a coordinator allocated one), `workspace`, the sandbox the executor acquired flattened beside them (`ir::SandboxInstance`: `provider` — `host`, `docker`, `daytona` or a plugin's kind —, `instance`, the provider's id for the sandbox, `image` and `snapshot` when the provider knows them, `working_directory`, the directory the scope's steps run in as the sandbox sees it), `duration_ms`, the acquisition's wall-clock time. Recorded by the driver once per acquisition, before any `step.started` in the scope; a resumed execution and an inherited sandbox's execution each record their own, naming the same lease and sandbox. No subject: a scope is not a node. Nothing derived |
...
| `command.started/completed` | `step.started` (the script is `subject.node.meta.script`), `step.finished {outcome}` (`metrics.exit_code`, `duration_ms`, `output`), `step.progress.recorded` (log lines and artifacts) | node, firing, attempt | durable | `embedding::the_workflow_runs_without_adapters_…`, `petri-fabro-steps::steps::the_public_stream_carries_the_script_and_the_matched_condition` |
