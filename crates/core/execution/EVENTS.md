...
| `agent.mcp.*` (C2): server and tool lifecycle | `step_custom` kinds `fabro.mcp.server` (`starting`, `ready` with the tools, `failed`, `stopped`; `ready` and `failed` mirror Pebble's `McpServerReady` and `McpServerFailed`) and `fabro.mcp.tool` (server, tool, tool call id, status, duration; derived from Pebble's `ToolCallStarted/Completed` for an `mcp__<server>__<tool>` name); the call itself is Pebble's `ToolCallStarted/Completed` in `agent_activity` under `mcp__<server>__<tool>` | node, firing, attempt, server name, tool call id | durable | `fabro_readiness_blackbox`, `embedding_readiness` (through `replay_run`); raw-log reads in `fabro_mcp_blackbox` |
