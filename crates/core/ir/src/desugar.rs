...
//! generations ([`sequential_for_each`]). Parallel iteration uses
//! [`Expansion`], which the engine splices into the live graph
//! ([`parallel_for_each`]).
//!
...
pub struct LoopExprs {
    ...
    pub more:   ExprId,
    /// `{ items, idx: idx + 1, acc: acc ++ [output] }` — the back edge's
    /// payload.
    pub next:   ExprId,
    /// `input.acc ++ [output]` — the exit edge's payload: every result, in
    /// order.
    pub result: ExprId,
}
...
pub struct SequentialForEach {
    ...
    pub back:  EdgeId,
    /// Edge from the loop tail to the collector, carrying every result in
    /// order.
    pub exit:  EdgeId,
}
...
///
/// `source` must output the items array. The body runs between `head` and
/// `tail` (they may be the same node) and must carry the loop state on its
/// payload; read the current element through [`LoopExprs::item`].
///
/// `head` is set to [`JoinPolicy::Any`], because on the first iteration only
/// the entry edge carries a token and on later ones only the back edge does.
/// Every node in the loop is capped at `max_iterations` firings, which is what
/// makes the run terminate (invariant 4).
...
/// [`sequential_for_each`] with an explicit `items` expression, evaluated in
/// the source node's outcome context, instead of the source's whole `output`.
...
pub struct CollectorExprs {
    /// `{ index, value: output }` — the payload each clone sends to the
    /// collector.
    pub indexed: ExprId,
    /// `pluck(sort_by_key(inputs, "index"), "value")` — every clone result,
    /// back in `items` order.
    pub ordered: ExprId,
}

/// Build the collector expressions. Put [`CollectorExprs::indexed`] on the edge
/// out of the expanded node, and [`CollectorExprs::ordered`] on the edge out of
/// the collector.
...
/// `index` bound, and every clone's outgoing edges are spliced into the
/// collector, so the collector's `All` join counts them all.
pub fn parallel_for_each(
    ...
    b.set_expansion(node, Expansion::ForEach {
        items,
        target,
        max_parallel,
        fail_fast,
