...
//!
//! An [`EventObserver`] is the seam every host consumer hangs off — a run store's
//! ingest, a progress UI, a checkpoint writer, a watchdog. The driver calls it
//! after every `apply` with each newly appended record and the post-apply state,
//! so a consumer resolves a firing to its node, name and `meta` in place
//! ([`EngineState::firing_node`]) instead of keeping a projection of its own.
//!
...
pub trait EventObserver: Send + Sync {
    ...
    /// `(log identity, seq)`: `seq` is unique only within one log, and the log's
    /// identity is host-named (a run dir, a store key), stable across resume,
    /// fresh per fork.
    ///
...
pub struct ObserveError {
    /// Which observer failed, in the observer's own words (`events.jsonl`, say).
    pub observer: String,
    pub message: String,
}
