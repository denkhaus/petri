...
fn an_old_log_version_is_rejected() {
    ...
    // outcomes route; v4: `Node.meta` inside splices; v5: `ControlRequested`), so
    // an old log is rejected cleanly rather than replayed under rules it was not
    // written for.
    for old in [1, 2, 3, 4] {
        let downgraded = encoded.replacen("\"version\":5", &format!("\"version\":{old}"), 1);
