...
environment's own `RUNNER_TOOL_CACHE` (a runner image ships a populated
`/opt/hostedtoolcache` and says so in its env), else the host's persistent
store where the step's filesystem has it, else the per-run workspace
directory.
...
| `ignored.services.ports` | `services.<id>.ports` publishes ports to the runner host. A job reaches a service as `<id>` on the scope's network, so no port is published and the list is dropped, naming the service. |
...
| `expression.runner_tool_cache` | `runner.tool_cache` under a function in a condition | The same runner-side truth again, with the same operator-position support. Read `RUNNER_TOOL_CACHE` in the step instead. |
...
| `action.upstream_gone` | A remote action whose repository is unavailable upstream — removed, or private to its org (recorded at snapshot refresh, or a live git refusal: `Repository not found`, a credential prompt with prompts disabled) | No anonymous local fetch can ever serve it (an org-private repository may still work on GitHub itself, granted org-side Actions access), so no refresh, and no feature work here, can change it. Kept out of the compatibility denominator ("broken upstream"). |
