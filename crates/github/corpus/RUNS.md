...
|---|---|---|
| passed | 150 | 62% |
| **failed on a runtime-tier gap** | 2 | 1% |
| expected failure (server-coupled) | 90 | 37% |
| **timed out** | 1 | 0% |
...
| `actions/download-artifact · cross-run artifact download (REST API)` | 5 |
...
| `actions/github-script · reads the simulated event's fabricated resource (nothing exists server-side as #999999)` | 4 |
...
| `actions/checkout · checks out a ref built from an empty dispatch input` | 3 |
| `actions/checkout · needs a repository secret` | 3 |
| `gr2m/create-or-update-pull-request-action · needs a repository secret` | 3 |
| `actions/setup-go · reads a stubbed script's output` | 2 |
| `ghcr.io/ossf/scorecard-action:v2.4.4 · signs results with the Actions-issued GITHUB_TOKEN` | 2 |
...
| `Dockerfile · reads a caller input left empty (a reusable workflow run standalone)` | 1 |
| `Dockerfile · reads the simulated event's fabricated resource (nothing exists server-side as #999999)` | 1 |
| `JamesIves/github-pages-deploy-action · pushes a deployment branch` | 1 |
| `PyO3/maturin-action · needs a tool GitHub's full runner image preinstalls (rustup)` | 1 |
...
| `dorny/paths-filter · reads the simulated event's fabricated resource (nothing exists server-side as #999999)` | 1 |
...
| `liskin/gh-problem-matcher-wrap · a stubbed script would have installed `flake8`` | 1 |
...
| `vercel/next.js/.github/actions/pr-auto-label · reads the simulated event's fabricated resource (nothing exists server-side as #999999)` | 1 |
...
| actions/checkout | `publish-immutable-actions.yml` | expected failure | `actions/publish-immutable-action` — step exited with status 1 · `Error: Could not find GITHUB_REPOSITORY_ID.` _(publishes to GitHub's registry)_ |
...
| astral-sh/uv | `bench.yml` | expected failure | `actions/github-script` — step exited with status 1 · `Error: Unhandled error: Error: Cannot find module '/workspace/.ci/github/actions/actions/github-script/ed597411d8f924073f98dfc5c65a23a2325f34cd/.codspeed-runner-installer-hashes.json' Require stack: -…` _(the inline script threw (it acts on the triggering event, which a local run lacks))_ |
...
| astral-sh/uv | `pull-request-labels.yml` | expected failure | `actions/checkout` — step exited with status 1 · `Error: The process '/usr/bin/git' failed with exit code 1` _(checks out a ref built from an empty dispatch input)_ |
...
| django/django | `labels.yml` | expected failure | `actions/github-script` — step exited with status 1 · `Error: Unhandled error: HttpError: Not Found - https://docs.github.com/rest/issues/labels#add-labels-to-an-issue` _(reads the simulated event's fabricated resource (nothing exists server-side as #999999))_ |
| django/django | `linters.yml` | expected failure | `liskin/gh-problem-matcher-wrap` — step exited with status 1 · `Error: Unable to locate executable file: flake8. Please verify either the file path exists or the file can be found within a directory specified by the PATH environment variable. Also check the file m…` _(a stubbed script would have installed `flake8`)_ |
| django/django | `new_contributor_pr.yml` | expected failure | `Dockerfile` — step exited with status 1 · `Error: Not Found - https://docs.github.com/rest/pulls/reviews#create-a-review-for-a-pull-request` _(reads the simulated event's fabricated resource (nothing exists server-side as #999999))_ |
...
| facebook/react | `runtime_release_from_ci.yml` | pass | — |
...
| facebook/react | `shared_close_direct_sync_branch_prs.yml` | expected failure | `actions/github-script` — step exited with status 1 · `Error: Unhandled error: HttpError: Not Found` _(reads the simulated event's fabricated resource (nothing exists server-side as #999999))_ |
...
| hashicorp/terraform | `enforce-changelog.yml` | expected failure | `dorny/paths-filter` — step exited with status 1 · `Error: Not Found` _(reads the simulated event's fabricated resource (nothing exists server-side as #999999))_ |
...
| hashicorp/terraform | `equivalence-test-manual-update.yml` | expected failure | `actions/checkout` — step exited with status 1 · `Error: The process '/usr/bin/git' failed with exit code 1` _(checks out a ref built from an empty dispatch input)_ |
...
| nodejs/node | `scorecard.yml` | expected failure | `ghcr.io/ossf/scorecard-action:v2.4.4` — step exited with status 1 · `{"date":"2026-08-31T01:13:15Z","repo":{"name":"github.com/nodejs/node","commit":"12e8e0b8d327549c719b5b09e462c4884f1fcaba"},"scorecard":{"version":"v5.5.0","commit":"c395761df6afe1a69e476bc60a013a94bc…` _(signs results with the Actions-issued GITHUB_TOKEN)_ |
...
| ohmyzsh/ohmyzsh | `scorecard.yml` | expected failure | `ghcr.io/ossf/scorecard-action:v2.4.4` — step exited with status 1 · `2026/08/31 01:16:49 error SigningNew: invalid token: not a default GITHUB_TOKEN` _(signs results with the Actions-issued GITHUB_TOKEN)_ |
| pola-rs/polars | `benchmark-remote.yml` | pass | — |
| pola-rs/polars | `benchmark.yml` | pass | — |
| pola-rs/polars | `changes-dsl-labeler.yml` | pass | — |
| pola-rs/polars | `clear-caches.yml` | pass | — |
| pola-rs/polars | `docs-python.yml` | pass | — |
...
| pola-rs/polars | `pr-labeler.yml` | expected failure | `actions/github-script` — step exited with status 1 · `Error: Unhandled error: HttpError: Not Found - https://docs.github.com/rest/issues/labels#add-labels-to-an-issue` _(reads the simulated event's fabricated resource (nothing exists server-side as #999999))_ |
| pola-rs/polars | `release-drafter.yml` | pass | — |
| pola-rs/polars | `release-rust.yml` | pass | — |
| pola-rs/polars | `test-bytecode-parser.yml` | pass | — |
| pola-rs/polars | `test-pyodide.yml` | expected failure | `PyO3/maturin-action` — step exited with status 1 · `Error: Unable to locate executable file: rustup. Please verify either the file path exists or the file can be found within a directory specified by the PATH environment variable. Also check the file m…` _(needs a tool GitHub's full runner image preinstalls (rustup))_ |
...
| prometheus/prometheus | `repo_sync.yml` | **fail** | `github/checkout` — step exited with status 1 · `Error response from daemon: container 50ce8a43021af223f78d40d63713c95604eefdbae133ce6c4bb38924c659c27e is not running` |
| prometheus/prometheus | `scorecards.yml` | expected failure | `github/codeql-action/upload-sarif` — step exited with status 1 · `Error: Not Found - https://docs.github.com/rest/actions/workflow-runs#get-a-workflow-run` _(uploads to GitHub code scanning)_ |
| prometheus/prometheus | `stale.yml` | pass | — |
| python/cpython | `add-issue-header.yml` | expected failure | `actions/github-script` — step exited with status 1 · `Error: Unhandled error: HttpError: Not Found - https://docs.github.com/rest/issues/issues#get-an-issue` _(reads the simulated event's fabricated resource (nothing exists server-side as #999999))_ |
...
| python/cpython | `reusable-check-html-ids.yml` | expected failure | `actions/download-artifact` — step exited with status 1 · `Error: Unable to download artifact(s): Artifact not found for name: html-ids-head.json.gz         Please ensure that your artifact is not expired and the artifact was uploaded using a compatible versi…` _(cross-run artifact download (REST API))_ |
| python/cpython | `reusable-cifuzz.yml` | expected failure | `Dockerfile` — step exited with status 1 · `config_utils.ConfigError: Invalid Configuration.` _(reads a caller input left empty (a reusable workflow run standalone))_ |
| python/cpython | `reusable-context.yml` | pass | — |
| python/cpython | `reusable-docs.yml` | pass | — |
...
| vercel/next.js | `pull_request_auto_label.yml` | expected failure | `vercel/next.js/.github/actions/pr-auto-label` — step exited with status 1 · `Error: HttpError: Not Found - https://docs.github.com/rest/pulls/pulls#list-pull-requests-files     at /workspace/.ci/github/actions/vercel/next.js/2fe6f962a1982594bdda96a7de16c594677266d2/.github/act…` _(reads the simulated event's fabricated resource (nothing exists server-side as #999999))_ |
