...
live-only field. The derivation is invertible for the records replay
consumes: the first event of every coordinator record and every external
engine record carries the whole record, `execution::events::invert` rebuilds
them from the stream, and `verify_lossless` (run beside `verify_replay` at
the end of a run) checks that the inverted records replay to the same logs
and the same stream. The core's own records are regenerated by replay, so
the stream need not carry them. The pure state machine reads no clock: the recording time
